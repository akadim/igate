 ...
 *
 */
class ArrausermigrateModelUtf extends JModel{
	/**
	...
	function csvExport($userType, $column, $order="", $mode_order=""){	  	   
		...
		if(strpos($columns, "usertype") !== false){
			$columns = str_replace("usertype", " GROUP_CONCAT(CAST(ugm.group_id as CHAR)) usertype_id ", $columns);
		}
		...
		$sql = "SELECT " . $columns .
		        " FROM #__users, #__usergroups ug, #__user_usergroup_map ugm ".$where." group by #__users.id ".$ordering;	
			...
			foreach($result as $key=>$value){
				...
				foreach($value as $key2=>$value2){
					...
					if($export_name == true && $split_name != "no" && $next == true){
						...
						if($key2 == "usertype_id"){
							$row .= $this->getGroupList($all_groups, trim($value2)).$separator;
						}
						elseif($key2 == "params"){
							$params = str_replace("\r\n", "***", trim($value2));
	...
	function sendMail($data, $file_name, $list_emails){
			...
			if(intval(JVERSION) >= 3){
				JMail::sendMail($from, $fromname, $recipient, $subject, $body, $mode, NULL, NULL, $attachment);
			}
			else{
