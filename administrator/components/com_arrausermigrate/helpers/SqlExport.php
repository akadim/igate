...
class SqlExport{
	...
	function getUsers($config){
		...
		$data .= "INSERT INTO `" . "#__" . "users` (`id`, `name`, `username`, `email`, `password`, `usertype`, `block`, `sendEmail`, `registerDate`, `lastvisitDate`, `activation`, `params`) VALUES\n";               		
		$i=0;
		$value_max_for_insert = 279;
		$separator = ",";
		if(is_array($result)){	
			...
			foreach($result as $key=>$value){
				...
				if($i == $value_max_for_insert){
					...
					$data .= "('".$value['id']."', '".addslashes($value['name'])."', '".addslashes($value['username'])."', '".addslashes($value['email'])."', '".$value['password']."', '".addslashes($value['usertype'])."', '".$value['block']."', '".$value['sendEmail']."', '".$value['registerDate']."', '".$value['lastvisitDate']."', '" .$value['activation']."', '".$value['params']."')".$separator."\n";
